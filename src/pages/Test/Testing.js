// import { SignJWT, generateSecret, importPKCS8, jwtVerify } from 'jose';
const { SignJWT, generateSecret, importPKCS8, jwtVerify } = require('jose');

const pkcs8 = `-----BEGIN PRIVATE KEY-----

ak6k+8+UFd93M4z0Qo74OhXB
-----END PRIVATE KEY-----`;
const alg = 'RS256';
const secretKey = await importPKCS8(pkcs8, alg);

const generateToken = {
  createAccess: async (payload) => {
    const token = await new SignJWT(payload)
      .setProtectedHeader({ alg })
      .setIssuedAt() // If no argument is used the current unix timestamp is used as the claim.
      .setIssuer('BEGoonco')
      .setAudience('FEGoonco')
      .setExpirationTime('5min')
      .sign(secretKey);
    return token;
  },

  createRefresh: async (payload) => {
    // const secretKey = await generateSecret('HS256');
    const token = await new SignJWT(payload)
      .setProtectedHeader({ alg })
      .setIssuedAt() // If no argument is used the current unix timestamp is used as the claim.
      .setIssuer('BEGoonco')
      .setAudience('FEGoonco')
      .setExpirationTime('1hour')
      .sign(secretKey);
    return token;
  },
};

const testingThis = async () => {
  const tok = await generateToken.createAccess({ id: 1 });
  const { payload, protectedHeader } = await jwtVerify(tok, secretKey, {
    issuer: 'urn:example:issuer',
    audience: 'urn:example:audience',
  });

  console.log(payload, protectedHeader);
};

testingThis();

export default generateToken;
